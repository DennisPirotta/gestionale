        ...
        <nav>
            <div class="nav nav-tabs mb-3" id="nav-tab" role="tablist">
                @foreach($statuses as $status)
                    <button class="nav-link" id="nav-{{str_replace(' ','',$status->description)}}-tab" data-bs-toggle="tab" data-bs-target="#nav-{{str_replace(' ','',$status->description)}}" type="button" role="tab" aria-controls="nav-{{str_replace(' ','',$status->description)}}" aria-selected="true">{{$status->description}}</button>
                @endforeach
            </div>
        </nav>
        <div class="tab-content" id="nav-tabContent">
            @foreach($statuses as $status)
                <div class="tab-pane fade show" id="nav-{{str_replace(' ','',$status->description)}}" role="tabpanel" aria-labelledby="nav-{{str_replace(' ','',$status->description)}}-tab" tabindex="0">
                    <div class="row">
                    @foreach($commesse as $commessa)
                        @if($status->id == $commessa->status)
                            <div class="col-sm-6 col-md-4 mb-3">
                                <div class="card h-100 bg-opacity-25 bg-{{DB::table('statuses')->where('id',$commessa['status'])->value('color')}}"> <!--  bg-black bg-opacity-25 -->
                                    <div class="card-body">
